...
from src.api.models.responses import ChatResponse, ChatMetadata, ErrorResponse
...
async def chat(
    ...
    try:
        ...
        try:
            ...
            if lowered.startswith("remember this") or lowered.startswith("remember:") or lowered.startswith("remember "):
                remember_match = request.message.split(":", 1)[-1].strip()
                if remember_match:
                    await memory_mgr.remember(
                        content=remember_match,
                        metadata={"session_id": session_id, "source": "explicit_remember"}
                    )
