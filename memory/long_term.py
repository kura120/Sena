        ...
        try:
            ...
            if self.db:
                ...
                result = await self.db.execute(
                    ...
                    (content, metadata, embedding, created_at, accessed_count, last_accessed)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (
                        content,
    ...
    async def search(
        ...
        try:
            ...
            rows = await self.db.fetch_all(
                """
                SELECT id, content, metadata, accessed_count, created_at, last_accessed
    ...
    async def get_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            count = await self.db.fetch_one(
                "SELECT COUNT(*) FROM memory_long_term"
            )
            
            most_accessed = await self.db.fetch_all(
                """
                SELECT id, content, accessed_count 
                FROM memory_long_term 
            ...
            return {
                ...
                "most_accessed": [
                    {"id": m[0], "content": m[1][:100], "accesses": m[2]}
                    for m in most_accessed
                ] if most_accessed else [],
                "recent": [
                    {"id": m[0], "content": m[1][:100], "created_at": m[2]}
                    for m in recent
                ] if recent else [],
            }
