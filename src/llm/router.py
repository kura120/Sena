...
class IntentRouter:
    ...
    def __init__(self) -> None:
        ...
        self._farewell_keywords = {
            ...
            "good night", "take care", "cya", "gtg",
        }
        
        self._code_keywords = {
            "code", "program", "function", "class", "implement",
            "write", "create", "build", "develop", "script",
        ...
        self._memory_indicators = {
            ...
            "before", "earlier", "you said", "we discussed",
            "mentioned", "told you", "forgot",
        }

        self._file_keywords = {
            "file", "files", "folder", "folders", "directory", "directories",
            "downloads", "desktop", "documents", "path", "filename",
        }

        self._file_action_keywords = {
            "find", "search", "locate", "check", "look for", "exists",
            "is there", "do i have", "in my",
        }
    ...
    def _quick_classify(self, input_lower: str) -> Optional[IntentResult]:
        ...
        # Check for questions
        if input_lower.endswith("?") or input_lower.startswith(("what", "who", "where", "when", "why", "how", "is", "are", "can", "could", "would", "should")):
            # Determine complexity
            if len(input_lower) > 100 or any(kw in input_lower for kw in ["analyze", "compare", "explain why", "in depth"]):
    ...
    async def _llm_classify(self, user_input: str) -> IntentResult:
        ...
        try:
            ...
            if not router_model:
                ...
                router_model = await self._registry.switch_to(ModelType.FAST)
