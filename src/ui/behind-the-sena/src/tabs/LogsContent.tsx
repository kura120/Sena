...
export const LogsContent: React.FC = () => {
  ...
  const parseRequestPayload = (value: string): Record<string, unknown> | null => {
  ...
  const groupChatLogs = (entries: LogEntry[]) => {
    ...
    for (const entry of sorted) {
      ...
      const isEnd = entry.message.includes('=== CHAT REQUEST COMPLETE ===') ||
      ...
      if (current) {
        ...
        const sessionId = extractSessionId({ ...entry, metadata: current.metadata })
        if (sessionId) {
  ...
  async function fetchLogs() {
    try {
      const url = new URL('http://127.0.0.1:8000/api/v1/logs')
      if (filter !== 'all') url.searchParams.append('level', filter)
      ...
      if (!selectedStillExists && !userSelectedRef.current && logList.length > 0) {
  ...
  const selectedLog = logs.find((log) => log.id === selectedId) || selectedSnapshotRef.current
  ...
  const getDetailMessage = (entry: LogEntry) => {
    if (entry.kind === 'chat' && entry.metadata?.request_payload && typeof entry.metadata.request_payload === 'object') {
      const payload = entry.metadata.request_payload as Record<string, unknown>
  ...
  const handleCopy = async () => {
    ...
    try {
      await navigator.clipboard.writeText(getMessageText(getDetailMessage(selectedLog)))
      setCopyState('copied')
        ...
        <div className="h-full grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-4">
                ...
                <div className="divide-y divide-slate-800/70">
                  {logs.map((msg) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${levelDot[msg.level]}`} />
          ...
          <div className="bg-slate-900 rounded-lg border border-slate-800 overflow-hidden flex flex-col">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3 space-y-4">
              {!selectedLog ? (
                <div className="text-sm text-slate-500">Select a log entry to view details.</div>
              ) : (
                <>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`w-2 h-2 rounded-full ${levelDot[selectedLog.level]}`} />
                    ...
                    <span className="text-xs text-slate-500">{selectedLog.source}</span>
                  </div>

                  <div>
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-slate-400 uppercase tracking-widest">Message</p>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="mt-2 rounded-md bg-slate-950/80 border border-slate-800/80 px-3 py-2">
                      {isJsonMessage(getDetailMessage(selectedLog)) ? (
                        ...
                        <div className="prose prose-invert max-w-none text-slate-100 text-sm">
                          <ReactMarkdown>{getDetailMessage(selectedLog)}</ReactMarkdown>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {selectedLog.kind === 'chat' && selectedLog.metadata && (
                    <div>
                      <p className="text-xs text-slate-400 uppercase tracking-widest">Chat timeline</p>
                        ...
                        <div className="space-y-2">
                          {selectedLog.children?.map((entry) => (
                            <div key={entry.id} className="flex items-start gap-2">
                              ...
                              <span className="text-slate-400">{entry.level.toUpperCase()}</span>
                  ...
                  {selectedLog.metadata && Object.keys(selectedLog.metadata).length > 0 && (
                    <div>
                      <p className="text-xs text-slate-400 uppercase tracking-widest">Metadata</p>
                      <div className="mt-2 rounded-md bg-slate-950/70 border border-slate-800/70 px-3 py-2 text-[11px] text-slate-200">
                  ...
                  {selectedLog.metadata?.request_payload && (
                    <div>
                      <p className="text-xs text-slate-400 uppercase tracking-widest">Request payload</p>
                      <div className="mt-2 rounded-md bg-slate-950/80 border border-slate-800/80 px-3 py-2">
                        <pre className="text-xs text-slate-100 font-mono whitespace-pre-wrap break-words">
                          {typeof selectedLog.metadata.request_payload === 'string'
                            ? selectedLog.metadata.request_payload
                            : JSON.stringify(selectedLog.metadata.request_payload, null, 2)}
                        </pre>
                      </div>
                    </div>
                  )}
