...
contextBridge.exposeInMainWorld('sena', {
  ...
  onServerLog: (callback: (message: string) => void) => {
    ipcRenderer.on('server-log', (_event, message: string) => callback(message))
  },

  onStartupStep: (callback: (data: { step: string }) => void) => {
    ipcRenderer.on('startup-step', (_event, data: { step: string }) => callback(data))
  },

  onStartupError: (callback: (data: { error: string }) => void) => {
    ipcRenderer.on('startup-error', (_event, data: { error: string }) => callback(data))
  },

  onWindowClosed: (callback: (windowId: string) => void) => {
    ipcRenderer.on('window-closed', (_event, windowId: string) => callback(windowId))
  },

  getHotkey: () =>
    ipcRenderer.invoke('get-hotkey'),

  setHotkey: (key: string) =>
    ipcRenderer.invoke('set-hotkey', key),
