...
export const ChatContent: React.FC = () => {
  const normalizeSessions = (items: ChatSession[]) => {
    return items.map((session, index) => {
      ...
      if (session.id.startsWith('session-') && session.id.split('-').length === 2) {
        return { ...session, title: session.title || `Session ${index + 1}` }
      }
      return { ...session, id: expectedId, title: session.title || `Session ${index + 1}` }
    })
  }
  ...
  const [sessions, setSessions] = useState<ChatSession[]>(() => {
    try {
      ...
      if (legacy) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          return normalizeSessions([{ id: 'session-1', title: 'Session 1', messages: parsed }])
        }
      }
    } catch (error) {
      console.warn('Failed to load chat history from storage:', error)
    }
    return normalizeSessions([{ id: 'session-1', title: 'Session 1', messages: [defaultMessage] }])
  })
  ...
  useEffect(() => {
    if (!sessions.find((session) => session.id === activeSessionId) && sessions.length > 0) {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const res = await fetch('http://127.0.0.1:8000/api/v1/chat', {
        ...
        body: JSON.stringify({ message: input, session_id: activeSession.id }),
      })
    ...
    <div className="h-full flex flex-col bg-[#0A0E27]">
      ...
      <div className="border-b border-slate-800/50 backdrop-blur-sm">
        <div className="max-w-3xl mx-auto w-full flex items-center gap-3 px-6 py-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-50">Chat with Sena</h2>
